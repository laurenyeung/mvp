...
export async function cleanDatabase() {
  ...
  await testPool.query(`DELETE FROM client_profiles WHERE user_id IN (SELECT id FROM users WHERE email LIKE '%_test@example.com')`)
  await testPool.query(`DELETE FROM coach_profiles WHERE user_id IN (SELECT id FROM users WHERE email LIKE '%_test@example.com')`)
